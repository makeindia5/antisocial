...
export default function GroupDiscussionScreen() {
    ...
    const { id, name } = useLocalSearchParams(); // Dynamic ID and Name from Router Params
    ...
    const fetchHistory = async () => {
        try {
            const res = await fetch(`${BACKEND_URL}/api/auth/gd/messages?groupId=${id}`);
            const data = await res.json();
    ...
    const fetchSummary = async (useCustomRange = false) => {
        ...
        try {
            let url = `${BACKEND_URL}/api/auth/gd/summary?groupId=${id}`;
            if (useCustomRange) {
    ...
    const setupSocket = () => {
        ...
        socket.current.on('connect', () => {
            ...
            socket.current.emit('joinGroup', id); // Join Dynamic Group Room
        });
    ...
    return (
        <View style={{ flex: 1, backgroundColor: styles.container.backgroundColor }}>
            <View style={[styles.header, { paddingTop: insets.top + 15 }]}>
                ...
                <TouchableOpacity onPress={() => { setInfoModalVisible(true); fetchUsers(); }} style={{ flexDirection: 'row', alignItems: 'center', flex: 1 }}>
                    <Text style={styles.headerText} numberOfLines={1}>{name || 'Group Discussion'}</Text>
                ...
                <View style={styles.headerActions}>
                    ...
                    <TouchableOpacity onPress={() => setCustomSummaryVisible(true)} style={styles.btnSummaryHeader}>
            ...
            <Modal animationType="fade" transparent={true} visible={customSummaryVisible} onRequestClose={() => setCustomSummaryVisible(false)}>
                <View style={styles.modalOverlay}>
                    <View style={styles.modalView}>
                        ...
                        <TouchableOpacity onPress={() => fetchSummary(true)} style={[GlobalStyles.button, { marginTop: 20 }]}>
                            <Text style={GlobalStyles.buttonText}>Generate Summary</Text>
                        </TouchableOpacity>
...
function getStyles(theme) {
    return StyleSheet.create({
        ...
        header: { backgroundColor: theme.primary, paddingTop: 15, paddingBottom: 15, paddingHorizontal: 15, flexDirection: 'row', alignItems: 'center' },
        headerText: { color: theme.white, fontSize: 18, fontWeight: 'bold' },
